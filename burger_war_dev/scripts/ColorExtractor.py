...
class ColorExtractor:
    ...
    def extract_red(self):
        red_mask = cv2.inRange(self.image, np.array(self.red_bgr_thresh["lower"]),np.array(self.red_bgr_thresh["upper"])) 
        return red_mask

    def extract_blue(self):
        blue_mask = cv2.inRange(self.image, np.array(self.blue_bgr_thresh["lower"]),np.array(self.blue_bgr_thresh["upper"])) 
        return blue_mask

    def extract_green(self):
        green_mask = cv2.inRange(self.image, np.array(self.green_bgr_thresh["lower"]),np.array(self.green_bgr_thresh["upper"]))
        return green_mask

    def extract_yellow(self):
        yellow_mask = cv2.inRange(self.image, np.array(self.yellow_bgr_thresh["lower"]),np.array(self.yellow_bgr_thresh["upper"])) 
        return yellow_mask
...
def main():
    ...
    image_list = glob.glob("../data/image_raws/1140*")
    ...
    image1 = color_extractor.extract_blue()
